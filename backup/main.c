...
enum
{
  ...
  DATA_MAX_HEADER = 50,     /* Maximum length of 'HEADER__' data in file */
  MAX_FILE_NAME = 100       /* Maximum file name length */
};
...
int extract_raw_block( FILE *infp, BlockDescr *block_descr )
{
  ...
  snprintf(file_name, MAX_FILE_NAME, "0x%04X%04X_%s_%li",
...
int main( int argc, char *argv[] )
{
  ...
  unsigned char header[DATA_MAX_HEADER];
  ...
  for (i = 0; i < nblocks; i++)
  {
    ...
    {
      /* Found header */
      int is_OGG = 0, is_uncompressed_WAVE = 0;
      ...
      if (blocks_descr[i].data_len == 17)
      {
        ...
        char magick[5], file_name[MAX_FILE_NAME];
        ...
        if (!strcmp(magick, "OggS"))
        {
          ...
          /* Creating file */
          snprintf(file_name, MAX_FILE_NAME, "0x%04X%04X.ogg",
      ...
      if (blocks_descr[i].data_len == 17 && header[0] == 0)
      {
        ...
        /* Creating file */
        snprintf(file_name, MAX_FILE_NAME, "2_0x%04X%04X.wav",
          (unsigned int)(((blocks_descr[i - 3].offset - 8 - 4 - 4) >> 16) & 0xFFFF),
          (unsigned int)((blocks_descr[i - 3].offset - 8 - 4 - 4) & 0xFFFF));
      ...
      if (blocks_descr[i].data_len == 21 || blocks_descr[i].data_len == 17)
      {
        long rate;         /* Sample rate */
        ...
        if (header[8] == 0)
        {
          if (blocks_descr[i].data_len == 21)
            start_offset = 0x7DC; /* FIXME: Check for 0x800 !!! */
          else /* 17 */
            start_offset = 0x800;
        ...
        /* Creating file */
        if (blocks_descr[i].data_len == 21)
          snprintf(file_name, MAX_FILE_NAME, "0x%04X%04X_%d.wav",
            (unsigned int)(((blocks_descr[i - 3].offset - 8 - 4 - 4) >> 16) & 0xFFFF),
            (unsigned int)((blocks_descr[i - 3].offset - 8 - 4 - 4) & 0xFFFF),
            header[17]);
        else
          snprintf(file_name, MAX_FILE_NAME, "1_0x%04X%04X.wav",
            (unsigned int)(((blocks_descr[i - 3].offset - 8 - 4 - 4) >> 16) & 0xFFFF),
            (unsigned int)((blocks_descr[i - 3].offset - 8 - 4 - 4) & 0xFFFF));
